...
AstroNN [@astronn] is a python package to do various kinds of neural networks with targeted application in astronomy. The AstroNN pipeline in `astra` is a convolutional neural network to estimate stellar labels ($T_\mathrm{eff}$, $\log{g}$, and chemical abundances) given some APOGEE spectrum.
...
```{python}
element = MODEL.c_h
element_lims = (-2.5, +0.5)
```

### With $T_{eff}$ 

```{python}
fig = qa.plot_abundances(
  MODEL.teff, 
  element, 
  MODEL,
  COADD_SPECTRUM_MODEL,
  x_lims=(3000, 8000),
  y_lims=element_lims,
)
```

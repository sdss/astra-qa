    ...
    for item in attributes:
        ...
        rows.append(
            (
                f"`{item}`", 
                f"{np.sum(mask):,}",
                f"{np.sum([ea or 0 for ea in sources['n_boss_visits'][mask]]):,}", 
                f"{np.sum([ea or 0 for ea in sources['n_apogee_visits'][mask]]):,}"
            )
        )
...
tabulate_counts_by_attribute(
    ...
    sorted(flags.all_alt_carton_names), 

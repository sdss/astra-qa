...
## There are multiple versions of APOGEENet

This page is showing results from APOGEENet II. [See here for APOGEENet III results.](pipelines_apogeenet_v3.html)
:::
...
APOGEENet [@apogeenet_1; @apogeenet_2] uses a convolutional neural network to estimate stellar labels ($T_\mathrm{eff}$, $\log{g}$, and metallicity [Fe/H]) given a rest-frame resampled APOGEE spectrum.
...
from astra.models import (ApogeeVisitSpectrum, ApogeeRestFrameVisitSpectrum, ApogeeCombinedSpectrum, ApogeeVisitSpectrumInApStar, ApogeeCoaddedSpectrumInApStar)
spectrum_models = (
    ApogeeVisitSpectrum,
...
qa.render_row_counts_by_spectrum_model_and_field(MODEL, spectrum_models, "apred")
```
...
::: {.callout-note}
The Sun is knowingly incorrect in APOGEENet II because no photometry or astrometry was given with the spectrum. It is flagged appropriately.
:::
...
try:
    ...
    Markdown(f"Failed to create figure")
```
...
### Calibrated error
```{python}
fig = qa.plot_z_scores("ApogeeNetV2")
```

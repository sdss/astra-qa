...
Snow White is a pipeline for classifying white dwarf spectra into their sub-types based on measured line ratios, and for fitting stellar parameters for DA-type white dwarfs.
...
fig = qa.plot_kiel_density(*data.T, x_lims=(3000, 80000), y_lims=(7, 10))
```
...
```{python}
fig = qa.plot_formal_error_wrt_snr(MODEL, (VISIT_SPECTRUM_MODEL, ), label_names=("teff", "logg"))
```
...
```{python}
fig = qa.plot_z_scores("SnowWhite")
```
